...
For standard mustelinet public access paths, do not try to restrict the security group source to the end user's public IP. Incoming HTTP/HTTPS and mustelinet SSH traffic is relayed by the mustelinet proxy or edge, so the VM sees the proxy as the source, not the original user IP.
...
For tenant SSH, use mustelinet SSH through the mustelinet public edge. The VM still needs an inbound security group rule allowing TCP port `22` from `0.0.0.0/0`.

SSH access also depends on:

- The VM having a floating IP.
- The VM image supporting SSH.
- The VM trusting the platform SSH User CA.
